...
function String:is( s )
    if s ~= nil then
        if type( s ) == 'string' then
            ...
            error( LOC( "$$$/X=String:is argument should be string, not ^1 (^2)", type( s ), tostring( s ) ), 2 ) -- 2 => assert error in calling context instead of this one.
        end
    else
        return false
    end
...
function String:getDriveAndSubPath( path )
    ...
    return root, table.concat( leaves, WIN_ENV and "\\" or "/" ) -- standardize file-path...
end
