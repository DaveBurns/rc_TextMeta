...
--
function Disk:copyFile( sourcePath, destPath, createDestDirsIfNecessary, overwriteDestFileIfNecessary, avoidUnnecessaryUpdate, timeCheckIsEnough )
    assert( self:existsAsFile( sourcePath ), "Source file does not exist: " .. sourcePath )
    return self:_transferFile( LrFileUtils.copy, sourcePath, destPath, createDestDirsIfNecessary, overwriteDestFileIfNecessary, avoidUnnecessaryUpdate, nil, timeCheckIsEnough )
end
...
function Disk:makeReadWrite( path )
    local s, m
    if WIN_ENV then
        --s, m = app:executeCommand( "ATTRIB", "-R", { path } ) - until 20/Oct/2012 17:53
        s = LrFileUtils.makeFileWritable( path ) -- ###1 trying @20/Oct/2012 17:54
        if not s then
            ...
            repeat
                ...
                if subPath then
                    ...
                    if LrFileUtils.isReadable( filePath ) then
                        local s, m, dc, ov, jt = fso:copyFile( filePath, destPath, true, overwrite, timeCheckIsEnough ) -- create dir, but error out if pre-existing file (dir should have been pre-deleted).
                        if s then
                            if jt then
...
function Disk:eject( drives )
    if WIN_ENV then
        local ejector = LrPathUtils.child( _PLUGIN.path, 'FreeEject.exe' ) -- ###1
