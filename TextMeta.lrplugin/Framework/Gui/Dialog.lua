...
--
function Dialog:promptForTargetPhotos( prefix, returnComponents, call )
    ...
    if #selectedPhotos == #auxPhotos then
        ...
        else -- multiple photos in filmstrip - all are selected
            buttons = { dia:btn( "Yes - Selected Photos", 'ok' ) }
            okPhotos = selectedPhotos
            confirm = "^1 ^2?"
            subs = { prefix, str:nItems( #selectedPhotos, "selected photos" ) }
            apk = str:fmtx( "^1 - selected photos", prefix )
        end
    ...
    elseif #selectedPhotos == 1 then -- multiple photos exist in filmstrip, of which only one is selected.
        ...
        buttons = { dia:btn( "Yes - Filmstrip Photos", 'ok' ), dia:btn( "Yes - Selected Photo Only", 'other' ) }
        otherPhotos = selectedPhotos
        okPhotos = auxPhotos
        confirm = "^1 all ^2 in filmstrip (will exclude photos buried in stacks), or just the one selected?"
        subs = { prefix, str:nItems( #auxPhotos, "photos" ) }
        apk = str:fmtx( "^1 - selected photo versus filmstrip", prefix )
    else
